...
// 비밀번호가 같은 경우 && 비밀번호 정규식
	...
	if($('#ecode').val()==$('#authNum').val()){
		inval_Arr[5] = true;
	} else {
		alert('인증번호를 정확히 확인해주세요');
		$('#ecode').focus();
		inval_Arr[5] = false;
	}
	// 휴대폰번호 정규식
	...
	//주소확인
	if($('#mb_address').val()!=''){
		if(address1J.test($('#mb_address1').val())){
			inval_Arr[7] = true;
		} else {
			inval_Arr[7] = false;
		}
	}else if($('#mb_address1').val()=='') {
		inval_Arr[7] = false;
...
$('#mb_address1').blur(function(){
	if(address1J.test($(this).val())){
		$("#adress_check").text('확인!');
		$('#adress_check').css('color', 'blue');
	} else {
		$('#adress_check').text('상세주소를 확인해주세요 ');
		$('#adress_check').css('color', 'red');
	}
});
}); // document.ready 종료
function eCheck() {
...
<body>
	<div class="wrapper">
		<div class="login-form">
			<form method="post" action="memberJoin" role="form" id="usercheck" name="member">
				<!-- 아이디 -->
				...
				<div class="form-group">
					<label for="mb_birth">생년월일</label> <input type="tel"
						...
						placeholder="ex) 19990101" maxlength="8">
				...
				<div class="form-group">
					<label for="mb_mobile">휴대폰 번호('-'없이 번호만 입력해주세요)</label> <input
						...
						name="mb_mobile" placeholder="Phone Number" maxlength="11">
				...
				<div class="form-group">
					...
					<input type="button" name="emailCheck" id="emailCheck"
						onclick="eCheck();" value="이메일 인증">
				</div>
				<div class="form-group">
					...
					<div class="eheck_font" id="ecode_check"></div>
				</div>
				<!-- 주소(우편) -->
				<div class="form-group">
				...
				<div class="form-group">
					<input class="form-control" style="top: 5px;"
						...
						<input class="form-control" placeholder="상세주소" name="mb_address1"
						id="mb_address1" type="text" />
						<div class="eheck_font" id="adress_check"></div>
				</div>
				<!-- 버튼 -->
				<div class="form-btn">
